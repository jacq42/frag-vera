# Frag Vera

## Purpose

The purpose of this application is to help users by planing their menus. What to eat today?

## Requirements

- Collection of recipes or meals with list of ingredients
- Content of refrigerator or freezer
- List of restaurants (optional)
- List of favorite meals per person (optional)
- Dietary restrictions (optional)
- Allergies (optional)
- Remember what leftovers are left for how many people (optional)

## Request

- "What to cook today?"
- "Create a menu plan for the next 7 days"
